...
export default function AuroraShader({
  colorStops = ["#ff7f7f", "#ffb3b3", "#cc99ff"],
  amplitude = 1.0,
  blend = 0.5,
  speed = 1.0,
}: AuroraProps) {
  ...
  useEffect(() => {
    ...
    gl.clearColor(0, 0, 0, 1); // solid black background
    gl.enable(gl.BLEND);
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full absolute inset-0 bg-black" // ensures background is black
    />
  );
}
